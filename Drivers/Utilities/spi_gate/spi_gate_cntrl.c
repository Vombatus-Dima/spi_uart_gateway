...
typedef enum
{
	NOTE_TRANSFER_CPLT = 0x00000001, NOTE_TRANSFER_HALF_CPLT = 0x00000002,
} note_spi_e;
...
uint8_t spiTxBuf[2][BUFF_SIZE] =
{ 0 };
uint8_t spiRxBuf[2][BUFF_SIZE] =
{ 0 };
...
void spiGateNote(note_spi_e noteValue)
{
	if (spiCntrlHandle != NULL)
	{
		...
		xTaskNotifyFromISR(spiCntrlHandle, noteValue, eSetBits,
				&IrqHighPrTskWoken);
...
void spiGateCntrlThread(void *arg)
{
	...
	{
		NoteValue = 0;
		if (xTaskNotifyWait(0x00000000, 0xFFFFFFFF, &(NoteValue),
				100) == pdFALSE)
		{ /* timed out */
...
void spiGateCntrlInit(void)
{
	...
	xTaskCreate(spiGateCntrlThread, (const char*) "SPI_cntrl",
			configMINIMAL_STACK_SIZE , NULL, TreadPrioHigh, &spiCntrlHandle);
}
